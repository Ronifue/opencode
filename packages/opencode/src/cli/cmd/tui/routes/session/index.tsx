        ...
        await sdk.client.session
          ...
          .then((res) =>
            Clipboard.copy(res.data!.share!.url).catch(() =>
              toast.show({ message: "无法将 URL 复制到剪贴板", variant: "error" }),
            ),
          )
    ...
    },
    {
      ...
      onSelect: async (dialog) => {
        try {
          ...
          toast.show({ message: "会话转录已复制到剪贴板！", variant: "success" })
        } catch (error) {
          toast.show({ message: "复制会话转录失败", variant: "error" })
        }
        dialog.clear()
      },
    },
      ...
      <Show when={text()}>
        ...
        >
          ...
          >
            ...
            >
              <text fg={theme.textMuted}>
                <span style={{ bg: theme.accent, fg: theme.backgroundPanel, bold: true }}> 队列中 </span>
              </text>
            </Show>
          </box>
        </box>
...
function Glob(props: ToolProps<typeof GlobTool>) {
  return (
    <InlineTool icon="✱" pending="正在查找文件..." complete={props.input.pattern} part={props.part}>
      通配符查询 "{props.input.pattern}" <Show when={props.input.path}>于 {normalizePath(props.input.path)} </Show>
      <Show when={props.metadata.count}>
        ({props.metadata.count} 个匹配项)
      </Show>
    </InlineTool>
  )
}
...
function Grep(props: ToolProps<typeof GrepTool>) {
  return (
    <InlineTool icon="✱" pending="正在搜索内容..." complete={props.input.pattern} part={props.part}>
      搜索内容 "{props.input.pattern}" <Show when={props.input.path}>于 {normalizePath(props.input.path)} </Show>
      <Show when={props.metadata.matches}>
        ({props.metadata.matches} 个匹配项)
      </Show>
    </InlineTool>
  )
}
...
function WebFetch(props: ToolProps<typeof WebFetchTool>) {
  return (
    <InlineTool icon="%" pending="正在从网络获取内容..." complete={(props.input as any).url} part={props.part}>
      网络获取 {(props.input as any).url}
    </InlineTool>
  )
}
...
function WebSearch(props: ToolProps<any>) {
  ...
  const metadata = props.metadata as any
  return (
    <InlineTool icon="◈" pending="正在搜索网页..." complete={input.query} part={props.part}>
...
function Task(props: ToolProps<typeof TaskTool>) {
  ...
  return (
    <Switch>
        ...
        >
          <box>
            <text style={{ fg: theme.textMuted }}>
              {props.input.description} ({tools().length} 次工具调用)
            </text>
            <Show when={current()}>
              {(item) => {
      ...
      <Match when={true}>
        <InlineTool icon="#" pending="正在委派任务..." complete={props.input.subagent_type} part={props.part}>
          {props.input.subagent_type} 任务 {props.input.description}
        </InlineTool>
      </Match>
    </Switch>
  )
...
function ApplyPatch(props: ToolProps<typeof ApplyPatchTool>) {
  ...
  function title(file: { type: string; relativePath: string; filePath: string; deletions: number }) {
    ...
    if (file.type === "move") return "# 已移动 " + normalizePath(file.filePath) + " → " + file.relativePath
    return "← 已应用补丁 " + file.relativePath
  }
  ...
  return (
    <Switch>
      ...
      <Match when={true}>
        <InlineTool icon="%" pending="正在准备应用补丁..." complete={false} part={props.part}>
          应用补丁 (apply_patch)
        </InlineTool>
      </Match>
    </Switch>
...
function Skill(props: ToolProps<typeof SkillTool>) {
  return (
    <InlineTool icon="→" pending="正在加载技能..." complete={props.input.name} part={props.part}>
      技能 "{props.input.name}"
    </InlineTool>
  )
}
